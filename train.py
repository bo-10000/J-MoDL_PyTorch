    ...
    for epoch in range(start_epoch, epochs):
        for phase in phases: #['train', 'val'] or ['train']
            ...
            for i, (gt, csm) in enumerate(tqdm(dataloaders[phase])):
                ...
                running_score['loss'] += loss.item() * gt.size(0)
            ...
            if args.write_image > 0 and (epoch % args.write_image == 0):
                ...
                writers[phase].add_figure('img', display_img(np.abs(x_u[-1].detach().cpu().numpy()), mask, \
                    gt[-1], x_rec[-1], epoch_score[val_score_name]), epoch)
