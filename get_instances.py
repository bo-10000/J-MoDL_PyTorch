...
def get_dataset(dataset_name, dataset_params, mode, verbose=True):
    if dataset_name == 'jmodl_dataset':
        from datasets.jmodl_dataset import jmodl_dataset
        dataset = jmodl_dataset(mode=mode, **dataset_params)
    if verbose:
...
def get_model(model_name, model_params, device):
    if model_name == 'base_jmodl':
        from models.jmodl import JMoDL
        model = JMoDL(**model_params)
